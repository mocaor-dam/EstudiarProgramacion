...
public class Juego {
    ...
    public Personaje buscarPersonaje(String nombre, TRaza raza) throws DBException {
        return personajes.stream().filter(personaje -> personaje.getNombre().equalsIgnoreCase(nombre)
                        && personaje.getRaza() == raza)
                .findFirst().orElseThrow(() -> new DBException("No hay personakes con estos atributos"));
    }


    public void agregarPersonaje(Personaje personaje) throws DBException {
        if (!personajes.add(personaje)) {
            throw new DBException("No se ha podido anadir el personaje");
        }
    }

    public void personajeConMasAtaques() throws DBException {
        int masAtaques = personajes.stream()
                .mapToInt(p -> p.getAtaques().size()).max().orElseThrow(() -> new DBException("no hay personajes"));

        personajes.stream().filter(personaje -> personaje.getAtaques().size() == masAtaques).forEach(System.out::println);

    }

    public void todosLosAtaquesOrdenadosNombre() {
        personajes.stream().flatMap(p -> p.getAtaques().stream().distinct()
                .sorted(Comparator.comparing(Ataque::getNombre))).forEach(System.out::println);
    }

    public void todosLosAtaquesOrdenadosDamage() {
        personajes.stream()
                .flatMap(personaje -> personaje.getAtaques().stream().sorted(Ataque::compareTo))
    ...
    public void atacar(Personaje p1, Personaje p2, String ataque) throws DBException {
       ...
       Ataque elegido = p1.mejorAtaque(ataque);
    ...
    public Map<TRaza, List<Personaje>> devuelveMapaRazas() {
        ...
        for (TRaza raza : TRaza.values()) {
            mapa.put(raza, personajes.stream()
                    .filter(p -> p.getRaza() == raza).toList());
        }
        return mapa;
    }
